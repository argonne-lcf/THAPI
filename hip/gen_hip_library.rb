...
def print_bitfield(name, enum)
  ...
  puts <<EOF
  #{to_class_name(name)} = hipbitmask #{to_ffi_name(name)},
...
def print_enum(name, enum)
  ...
  puts <<EOF
  #{to_class_name(name)} = hipenum #{to_ffi_name(name)},
    [ #{members.collect(&print_lambda).join(",\n      ")} ]
...
module FFI
  ...
  class HIPUnion < Union
    def self.enclosing_module
      HIP
    end
  end
  class HIPBitmask < Bitmask
    def default=(default)
      @default = default
    end
  ...
  module Library
    def hipbitmask(*args)
      generic_enum(FFI::HIPBitmask, *args)
    end
    def hipenum(*args)
      generic_enum(FFI::HIPEnum, *args)
    end
  end
...
def print_union(name, union)
  ...
  puts <<EOF
  class #{to_class_name(name)} < FFI::HIPUnion
...
def print_struct(name, struct)
  ...
  puts <<EOF
  class #{to_class_name(name)} < FFI::HIPStruct
EOF
