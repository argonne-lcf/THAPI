...
STRUCT_TYPES = model[:event_classes].map do |f|
  f[:payload].filter_map do |l|
    native = l[:class]
    cast = l[:cast_type]
...
def get_field_class_properties(field, member_name)
  ...
  if field_properties[:type] == 'array_dynamic'
    field_properties[:length_field_path] =
      "EVENT_PAYLOAD[\"_#{member_name}_length\"]"
  end
...
def get_event_class_member(member)
  ...
  d = { name: member_name, field_class: get_field_class_properties(member, member_name) }
  d[:metadata] = { be_class: member[:be_class] } if member[:be_class]
...
def get_stream_class(sc)
  ...
  stream_class[:default_clock_class] = {} if sc.key?(:clock_snapshot_value)
