...
static bool is_elf64(const char *path) {
    ...
    bool ok = read(fd, ident, EI_NIDENT) == EI_NIDENT &&
...
static std::string resolve_versioned_name(std::string_view found_path, std::string_view libname) {
    ...
    assert(fname.size() > libname.size() &&
           fname.compare(0, libname.size(), libname) == 0 &&
           fname[libname.size()] == '.');
    return fname;
}
...
static bool version_check(std::string_view found_path, std::string_view libname, int soversion) {
    ...
    if (soversion < 0) {
        std::cerr << "Warning: no soversion specified for " << libname
                  << " but found versioned " << fname << "\n";
        return true;
    }

    int found_ver = std::stoi(fname.substr(libname.size() + 1));
    if (found_ver != soversion) {
        std::cerr << "Warning: expected " << libname << "." << soversion
                  << " but found " << fname << "\n";
        return true;
    }
    return false;
...
public:
    ...
    template<typename T>
    const T *array_at(size_t offset, size_t count) const {
...
template <size_t N>
void copy_to_buf(char (&dest)[N], std::string_view src) {
...
static bool for_each_dir(std::string_view pathlist, Fn &&fn) {
...
static bool build_path(char *buf, size_t buf_size,
...
static void search_pathlist_batch(std::string_view pathlist,
                                  ...
                                  int &pending) {
    if (pathlist.empty() || pending == 0) return;
    char buf[4096];
    for_each_dir(pathlist, [&](std::string_view dir) {
        for (size_t i = 0; i < entries.size(); i++) {
            if (results[i].path[0]) continue;
            bool found = false;
            ...
            if (!found) {
                if (entries[i].libname.empty()) continue;
...
static void search_cache_batch(const std::vector<batch_entry> &entries,
    ...
    if (pos + sizeof(cache_file_new) > mf.size()) return;
    ...
    for (uint32_t i = 0; i < cache->nlibs && pending > 0; i++) {
        ...
        for (size_t j = 0; j < entries.size(); j++) {
            ...
            if (!entries[j].needed_name.empty() && std::strcmp(key, entries[j].needed_name.data()) == 0) {
                ...
                if (access(val, F_OK) == 0) fallbacks[j] = val;
            }
        }
    }
...
static void parse_binary(const char *binary,
    ...
    for (size_t i = 0; i < dyn_count && dyns[i].d_tag != DT_NULL; i++) {
        switch (dyns[i].d_tag) {
        case DT_STRTAB:   strtab_vaddr = dyns[i].d_un.d_ptr; break;
        case DT_RUNPATH:  rpath_off = dyns[i].d_un.d_val; has_rpath = true; break;
        case DT_RPATH:    if (!has_rpath) { rpath_off = dyns[i].d_un.d_val; has_rpath = true; } break;
        default: break;
        }
    }
...
static void find_lib_batch(const char *binary,
                           const find_lib_request *requests, int count,
                           find_lib_result *results) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!results[i].path[0])
            ...
            results[i].status = version_check(results[i].path, requests[i].libname, requests[i].soversion) ? 2 : 0;
    }
}
