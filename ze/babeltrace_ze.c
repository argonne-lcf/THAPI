        ...
        bt_self_component_sink *self_component_sink)
{
    ...
    for (uint64_t i = 0; i < message_count; i++) {
        ...
        if (bt_message_get_type(message) == BT_MESSAGE_TYPE_EVENT) {
            ...
            const struct ze_unique_id  id = {
                bt_event_class_get_id(event_class),
                bt_stream_get_id(bt_event_borrow_stream_const(event)),
                bt_stream_class_get_id(stream_class) };
            struct ze_callbacks *callbacks = NULL;

            HASH_FIND(hh, ze_dispatch->callbacks, &id, sizeof(id), callbacks);
            if (!callbacks) {
                callbacks = (struct ze_callbacks *)calloc(1, sizeof(struct ze_callbacks));
                callbacks->id = id;
                HASH_ADD(hh, ze_dispatch->callbacks, id, sizeof(id), callbacks);
                struct ze_event_callbacks *event_callbacks = NULL;
                HASH_FIND_STR(ze_dispatch->event_callbacks, bt_event_class_get_name(event_class), event_callbacks);
                if (event_callbacks) {
