...
def check_group_property_queued(state, ctx, payload, device)
...
def check_valid_ordinal(state, ctx, payload, cqg_ordinal)
...
def check_valid_command_queue(state,ctx,payload, cmd_queues, cmd_queue_ptr)
...
def check_valid_module(state,ctx,payload)
...
def check_list_and_fence_have_matching_context(state,ctx,payload,cmd_list,fence)
  if fence 
...
def check_fence_and_queue_compatibility(state,ctx,payload,cmd_queue,fence)
  if fence 
...
def check_list_and_queue_have_matching_context(state,ctx,payload,cmd_list, cmd_queue)
...
def check_use_after_free_on_append(state, ctx, params, waits)
  if state.waits_satisfied?(ctx, waits)
...
def check_circular_deadlock(state, units)
  ...
  # DFS; stop at the first cycle and report only that one. on_path holds the
  # current branch (reaching one again closes a cycle), visited the rest.
  path = []
  on_path = {}
  visited = {}
  found = nil
...
def check_struct_stype_misuse(state,ctx,payload,expected_stype, observed_stype)
