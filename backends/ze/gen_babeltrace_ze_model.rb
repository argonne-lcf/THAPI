...
# A length this side never reads: a variable-length blob takes its count from
# the companion length field instead.
UNREAD_LENGTH = '0'
...
def gen_struct_event_bt_model(registry, provider, struct)
  ...
  gen_bt_event(registry, provider, struct,
                ...
                *field_types_name('lttng_ust_field_variable_length_blob', "#{struct} *", 'p_val', bytes)])
end
