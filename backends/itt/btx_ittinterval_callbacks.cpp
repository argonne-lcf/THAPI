...
static std::string format_numeric_meta(__itt_metadata_type type, size_t count,
                                       void *data_val_bytes) {
  switch (type) {
...
static inline void push_meta_message(void *btx_handle, const char *hostname, int64_t vpid,
                                     uint64_t vtid, int64_t ts, const std::string &scope_tag,
...
static inline void attach_to_current_task_meta(data_t *state, const char *hostname, int64_t vpid,
                                               uint64_t vtid, __itt_domain *domain,
                                               const std::string &key, const std::string &value) {
...
//   \_ (_| | | |_) (_| (_ |< _>
//
static void lttng_ust_itt___itt_string_handle_create_callback(void *btx_handle, void *usr_data,
...
static void lttng_ust_itt___itt_task_begin_callback(void *btx_handle, void *usr_data, int64_t ts,
                                                    ...
                                                    uint64_t vtid, __itt_domain *domain, __itt_id,
                                                    __itt_id, __itt_string_handle *name) {
  auto *state = static_cast<data_t *>(usr_data);
...
static void lttng_ust_itt___itt_event_create_callback(void *btx_handle, void *usr_data,
                                                      int64_t /*ts*/, const char *hostname,
                                                      int64_t vpid, uint64_t /*vtid*/,
                                                      __itt_event event, char *name, int namelen,
                                                      char *name_val) {
  auto *state = static_cast<data_t *>(usr_data);
...
static void lttng_ust_itt___itt_metadata_add_callback(
    void *btx_handle, void *usr_data, int64_t ts, const char *hostname, int64_t vpid, uint64_t vtid,
    __itt_domain *domain, __itt_id /*id*/, __itt_string_handle *key, __itt_metadata_type type,
    size_t count, void * /*data*/, size_t /*data_len*/, void *data_val) {
  auto *state = static_cast<data_t *>(usr_data);
...
static void lttng_ust_itt___itt_metadata_add_with_scope_callback(
    void *btx_handle, void *usr_data, int64_t ts, const char *hostname, int64_t vpid, uint64_t vtid,
    __itt_domain *domain, __itt_scope scope, __itt_string_handle *key, __itt_metadata_type type,
    size_t count, void * /*data*/, size_t /*data_len*/, void *data_val) {
  auto *state = static_cast<data_t *>(usr_data);
  if (type == __itt_metadata_unknown || count == 0 || !data_val)
    return;
