...
enum thapi_mpi_buffer_role { THAPI_MPI_ROLE_IN = 0, THAPI_MPI_ROLE_OUT = 1 };
...
typedef _thapi_ze_result_t (*_thapi_ze_mem_get_alloc_properties_fn_t)(
    _thapi_ze_context_handle_t hContext,
    const void *ptr,
...
typedef _thapi_ze_result_t (*_thapi_ze_mem_get_address_range_fn_t)(
    _thapi_ze_context_handle_t hContext, const void *ptr, void **pBase, size_t *pSize);
...
typedef _thapi_hip_error_t (*_thapi_hip_mem_get_address_range_fn_t)(void **pbase,
                                                                    size_t *psize,
                                                                    void *dptr);
...
static __thread struct _thapi_mpi_cache_entry _thapi_mpi_cache[_THAPI_MPI_CACHE_SLOTS];
static __thread unsigned int _thapi_mpi_cache_next = 0;

static inline int _thapi_mpi_cache_lookup(const void *ptr, struct _thapi_mpi_cache_entry *out) {
...
static inline void _thapi_mpi_cache_insert(const struct _thapi_mpi_cache_entry *e) {
  ...
  _thapi_mpi_cache_next = (_thapi_mpi_cache_next + 1) % _THAPI_MPI_CACHE_SLOTS;
}

/* --- Runtime probing ------------------------------------------------------ */
static int _try_classify_ze(const void *ptr, struct _thapi_mpi_cache_entry *out) {
  ...
  void *base = NULL;
  size_t size = 0;
  if (_ze_mem_get_address_range && _ze_mem_get_address_range(_ze_introspection_ctx, ptr, &base,
                                                             &size) != _THAPI_ZE_RESULT_SUCCESS) {
    base = (void *)ptr;
    size = 1;
  }
  ...
  switch (props.type) {
  case _THAPI_ZE_MEMORY_TYPE_HOST:
    out->kind = THAPI_MPI_PTR_HOST;
    break;
  case _THAPI_ZE_MEMORY_TYPE_DEVICE:
    out->kind = THAPI_MPI_PTR_DEVICE;
    break;
  case _THAPI_ZE_MEMORY_TYPE_SHARED:
...
static int _try_classify_cuda(const void *ptr, struct _thapi_mpi_cache_entry *out) {
  ...
  if (_cu_pointer_get_attribute(&mem_type, _THAPI_CU_POINTER_ATTRIBUTE_MEMORY_TYPE, dptr) !=
          _THAPI_CU_SUCCESS ||
      mem_type == 0)
    return 0;

  (void)_cu_pointer_get_attribute(&range_start, _THAPI_CU_POINTER_ATTRIBUTE_RANGE_START, dptr);
  (void)_cu_pointer_get_attribute(&range_size, _THAPI_CU_POINTER_ATTRIBUTE_RANGE_SIZE, dptr);
  (void)_cu_pointer_get_attribute(&device_ordinal, _THAPI_CU_POINTER_ATTRIBUTE_DEVICE_ORDINAL,
                                  dptr);
  ...
  switch (mem_type) {
  case _THAPI_CU_MEMORYTYPE_HOST:
    out->kind = THAPI_MPI_PTR_HOST;
    break;
  case _THAPI_CU_MEMORYTYPE_DEVICE:
    out->kind = THAPI_MPI_PTR_DEVICE;
    break;
  case _THAPI_CU_MEMORYTYPE_UNIFIED:
    out->kind = THAPI_MPI_PTR_SHARED;
    break;
  default:
    out->kind = THAPI_MPI_PTR_UNKNOWN;
...
static int _try_classify_hip(const void *ptr, struct _thapi_mpi_cache_entry *out) {
...
static void _load_gpu_introspection_once(void) { _load_gpu_introspection(); }
...
static inline void _dump_buffer_info(const void *ptr, int role) {
  ...
  if (!_thapi_mpi_cache_lookup(ptr, &e)) {
    e.valid = 0;
    if (!_try_classify_ze(ptr, &e) && !_try_classify_cuda(ptr, &e) && !_try_classify_hip(ptr, &e)) {
  ...
  do_tracepoint(lttng_ust_mpi_properties, buffer_info, ptr, role, (int)e.kind, (int)e.backend,
...
static void _load_gpu_introspection(void) {
  ...
  if (h_ze) {
    _thapi_ze_init_fn_t ze_init = (_thapi_ze_init_fn_t)dlsym(h_ze, "zeInit");
    _thapi_ze_driver_get_fn_t ze_driver = (_thapi_ze_driver_get_fn_t)dlsym(h_ze, "zeDriverGet");
    _thapi_ze_context_create_fn_t ze_ctx_new =
        (_thapi_ze_context_create_fn_t)dlsym(h_ze, "zeContextCreate");
    _ze_mem_get_alloc_properties =
        (_thapi_ze_mem_get_alloc_properties_fn_t)dlsym(h_ze, "zeMemGetAllocProperties");
    _ze_mem_get_address_range =
        (_thapi_ze_mem_get_address_range_fn_t)dlsym(h_ze, "zeMemGetAddressRange");
    ...
    struct {
      ...
      const void *pNext;
      uint32_t flags;
    } desc = {0x000d /* ZE_STRUCTURE_TYPE_CONTEXT_DESC */, NULL, 0};
    if (!(ze_init && ze_driver && ze_ctx_new &&
          ...
          ze_ctx_new(hDrv, &desc, &_ze_introspection_ctx) == _THAPI_ZE_RESULT_SUCCESS)) {
  ...
  if (h_cu) {
    _thapi_cu_init_fn_t cu_init = (_thapi_cu_init_fn_t)dlsym(h_cu, "cuInit");
    _cu_pointer_get_attribute =
        (_thapi_cu_pointer_get_attribute_fn_t)dlsym(h_cu, "cuPointerGetAttribute");
    if (!(cu_init && _cu_pointer_get_attribute && cu_init(0) == _THAPI_CU_SUCCESS))
  ...
  if (h_hip) {
    _hip_pointer_get_attributes =
        (_thapi_hip_pointer_get_attributes_fn_t)dlsym(h_hip, "hipPointerGetAttributes");
    _hip_mem_get_address_range =
        (_thapi_hip_mem_get_address_range_fn_t)dlsym(h_hip, "hipMemGetAddressRange");
  }
}
