...
# ---------------------------------------------------------------------------
# GPU-aware MPI introspection
#
# For every MPI call that carries a user payload buffer, register a prologue
# that emits an `lttng_ust_mpi_properties:buffer_info` event per buffer.  The
# event records whether the pointer is host / device / shared memory and which
# GPU runtime (Level Zero, CUDA, HIP) owns it.  Consumers (extract/,
# btx_mpiinterval_callbacks.cpp) use that to tag each MPI call as "GPU-aware"
# or "CPU-only".
#
# The set of buffer parameters is derived from mpi_meta_parameters.yaml so
# new MPI APIs picked up by codegen are classified automatically: any
# InScalar/OutScalar parameter whose name matches one of the canonical MPI
# payload-buffer names below is treated as a user buffer.
# ---------------------------------------------------------------------------
MPI_PAYLOAD_BUFFER_NAMES = %w[
  buf buffer sendbuf recvbuf inbuf outbuf inoutbuf
...
$mpi_meta_parameters.fetch(:meta_parameters, {}).each do |func, list|
  ...
  next if buffers.empty?

  body = buffers.map do |type, name|
    role = type == 'InScalar' ? 'THAPI_MPI_ROLE_IN' : 'THAPI_MPI_ROLE_OUT'
    "  _dump_buffer_info((const void *)#{name}, #{role});"
  end.join("\n")
