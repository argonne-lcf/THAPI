 ...
 */
static
bt_component_class_initialize_method_status opencl_text_initialize(
        bt_self_component_sink *self_component_sink,
        bt_self_component_sink_configuration *configuration,
        const bt_value *params, void *initialize_method_data)
{
    /* Allocate a private data structure */
    struct opencl_text *opencl_text = malloc(sizeof(struct opencl_text));
 ...
 */
static
void opencl_text_finalize(bt_self_component_sink *self_component_sink)
{
    /* Retrieve our private data from the component's user data */
    struct opencl_text *opencl_text = bt_self_component_get_data(
        bt_self_component_sink_as_self_component(self_component_sink));
...
opencl_text_graph_is_configured(bt_self_component_sink *self_component_sink)
{
    /* Retrieve our private data from the component's user data */
    struct opencl_text *opencl_text = bt_self_component_get_data(
        bt_self_component_sink_as_self_component(self_component_sink));
    ...
    bt_message_iterator_create_from_sink_component(self_component_sink,
        in_port, &opencl_text->message_iterator);
 ...
 */
static
void print_message(struct opencl_text *opencl_text, const bt_message *message)
{
    ...
    printf("#%" PRIu64 ": %s (id: %" PRIu64 ") (%" PRIu64 " payload member%s)\n",
        opencl_text->index, bt_event_class_get_name(event_class),
 ...
 */
static
bt_component_class_sink_consume_method_status opencl_text_consume(
        bt_self_component_sink *self_component_sink)
{
    ...
    bt_message_iterator_next_status next_status =
        bt_message_iterator_next(opencl_text->message_iterator, &messages,
            &message_count);

    switch (next_status) {
    case BT_MESSAGE_ITERATOR_NEXT_STATUS_END:
        /* End of iteration: put the message iterator's reference */
        bt_message_iterator_put_ref(opencl_text->message_iterator);
    ...
    for (uint64_t i = 0; i < message_count; i++) {
        ...
        print_message(opencl_text, message);
...
/* Define the `opencl_text` plugin */
BT_PLUGIN(opencl);

/* Define the `text` sink component class */
BT_PLUGIN_SINK_COMPONENT_CLASS(text, opencl_text_consume);

/* Set some of the `text` sink component class's optional methods */

BT_PLUGIN_SINK_COMPONENT_CLASS_INITIALIZE_METHOD(text, opencl_text_initialize);
BT_PLUGIN_SINK_COMPONENT_CLASS_FINALIZE_METHOD(text, opencl_text_finalize);
BT_PLUGIN_SINK_COMPONENT_CLASS_GRAPH_IS_CONFIGURED_METHOD(text, opencl_text_graph_is_configured);
