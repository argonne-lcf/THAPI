...
@test "row 2: no sover, no DT_NEEDED, unversioned on disk → found (exit 0)" {
  gcc -shared -o "$BATS_TEST_TMPDIR/libbar.so" "$BATS_TEST_DIRNAME/bar.c"
  echo 'int main(void){return 0;}' | gcc -x c -o "$BATS_TEST_TMPDIR/noop" -
...
@test "row 4: no sover, DT_NEEDED unversioned → found (exit 0)" {
  gcc -shared -o "$BATS_TEST_TMPDIR/libbar.so" "$BATS_TEST_DIRNAME/bar.c"
  gcc -o "$BATS_TEST_TMPDIR/foo" "$BATS_TEST_DIRNAME/foo.c" -L"$BATS_TEST_TMPDIR" -lbar -Wl,-rpath,"$BATS_TEST_TMPDIR"
...
@test "row 5: no sover, DT_NEEDED versioned → warning (exit 2)" {
  gcc -shared -Wl,-soname,libbar.so.1 -o "$BATS_TEST_TMPDIR/libbar.so.1" "$BATS_TEST_DIRNAME/bar.c"
...
@test "find_lib resolves \$ORIGIN in rpath" {
  ...
  gcc -shared -o "$BATS_TEST_TMPDIR/lib/libbar.so" "$BATS_TEST_DIRNAME/bar.c"
...
@test "find_lib prefers rpath over LD_LIBRARY_PATH" {
  ...
  gcc -shared -Wl,-soname,libbar.so.1 -o "$BATS_TEST_TMPDIR/rpath_dir/libbar.so.1" "$BATS_TEST_DIRNAME/bar.c"
  gcc -shared -Wl,-soname,libbar.so.1 -o "$BATS_TEST_TMPDIR/ldpath_dir/libbar.so.1" "$BATS_TEST_DIRNAME/bar.c"
...
@test "dlopen preserves symlinked directory in path (no rpath)" {
  ...
  gcc -shared -Wl,-soname,libbar.so.1 -o "$BATS_TEST_TMPDIR/real_dir/libbar.so.1" "$BATS_TEST_DIRNAME/bar.c"
...
@test "find_lib locates versioned library via ldconfig conf paths with DT_NEEDED" {
  ...
  ln -s libbar.so.1 "$BATS_TEST_TMPDIR/libdir/libbar.so"
