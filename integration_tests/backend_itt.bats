...
@test "ITT (C): trace contains __itt_task_begin events" {
  gcc ./integration_tests/itt_example.c -I${ITTAPI_ROOT}/include -L${ITTAPI_ROOT}/lib64 \
    -L${ITTAPI_ROOT}/lib -littnotify -ldl -o itt_example
...
@test "ITT (Python, context manager): trace contains ITT task events" {
  iprof --backends itt --analysis-output ./itt_out_py_context-manager.txt -- \
    python3 ./integration_tests/itt_example_context-manager.py
...
@test "ITT (Python, C-style): trace contains ITT task events" {
  iprof --backends itt --analysis-output ./itt_out_py_c-style.txt -- \
    python3 ./integration_tests/itt_example_c-style.py
