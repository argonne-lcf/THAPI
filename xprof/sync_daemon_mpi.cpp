...
const int RT_SIGNAL_READY = SIGRTMIN; // out
const int RT_SIGNAL_GLOBAL_BARRIER = SIGRTMIN + 1; // in
const int RT_SIGNAL_LOCAL_BARRIER = SIGRTMIN + 2; // in
const int RT_SIGNAL_FINISH = SIGRTMIN + 3; // in
		...
		if (global_rank != 0) {
		  MPI_Send(&local_size, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
			...
			while (sum_local_size_recv != global_size) {
				std::cout << sum_local_size_recv << std::endl;
				int local_size_recv;
				MPI_Recv(&local_size_recv, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
