...
template <typename T>
std::string to_string_with_precision2(const T a_value, const std::string extension, const int n = 2)
{
...
class TallyCoreBase
{
    public:
       ...
       bool operator> (const TallyCoreBase& rhs) {
...
class TallyCoreTime: public TallyCoreBase{
    ...
    private:
        template <typename T>
        std::string format_time2(const T duration) {
...
class TallyCoreByte: public TallyCoreBase{
    ...
    private:
        template <typename T>
        std::string format_byte2(const T duration) {
...
auto sort_by_value(Map<K,V>& m) {
    ...
    std::copy(m.begin(), m.end(),
              std::back_inserter<std::vector<std::pair<K, V>>>(v));
...
template< typename TC,
          typename = std::enable_if_t<std::is_base_of_v<TallyCoreBase, TC>>>
void apply_sizelimit(std::vector<std::pair<thapi_function_name, TC>> &m, 
                     int max_name_size) {
   for (auto& [k,v]: m) {
...
void print_tally(std::ostream& os, const std::tuple<T...>& s, const std::tuple<T2...>& h, std::index_sequence<I...>) {
    (( (std::get<I>(s).size()) ?
        os << std::get<I>(s).size() << " " << std::get<I>(h) << " | "
       :
        os << ""
      ),
      ...
...
template<class ...T, class ...T2>
void print_named_tuple(std::ostream& os, const std::string& header, const std::tuple<T...>& s, const std::tuple<T2...>& h) {
    os << header << " | ";
    constexpr auto seq = std::make_index_sequence<sizeof ...(T2)>();
...
template<typename TC,
         typename = std::enable_if_t<std::is_base_of_v<TallyCoreBase, TC>>>
void print_tally2(std::unordered_map<thapi_function_name, TC>& m,
...
template<typename K, typename T, typename TC,
         typename = std::enable_if_t<std::is_base_of_v<TallyCoreBase, TC>>>
void print_compact(std::string title,
                   std::unordered_map<K,TC> m,
                   T&& keys_string,
                   int display_name_max_size){
...
template<typename K, typename T, typename TC, 
         typename = std::enable_if_t<std::is_base_of_v<TallyCoreBase, TC>>>
void print_extended(std::string title,
                   std::unordered_map<K,TC> m,
                   T&& keys_string,
                   int display_name_max_size){
...
void to_json(nlohmann::json& j, const TallyCoreTime& tc) {
   j = nlohmann::json{{"time", tc.duration}, {"call", tc.count},
...
void to_json(nlohmann::json& j, const TallyCoreByte& tc) {
   j = nlohmann::json{{"size", tc.duration}, {"call", tc.count},
            {"min", tc.min}, {"max", tc.max} };
   if (tc.error != 0)
     j["error"] = tc.error;
}
...
template<typename K, typename TC,
         class ...T, 
         typename = std::enable_if_t<std::is_base_of_v<TallyCoreBase, TC>>>
nlohmann::json json_extented(std::unordered_map<K,TC>& m,
                   std::tuple<T...>&& h){
      nlohmann::json j;
      auto aggregated_nested = aggregate_nested(m);
      for (auto &[k,aggregated_by_name]: aggregated_nested) {
