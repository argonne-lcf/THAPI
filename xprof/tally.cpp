...
static inline void get_common_lltng(const bt_event *event, hostname_t &hostname,
                                    process_id_t &process_id, thread_id_t &thread_id,
                                    std::string &name) {
  ...
  const bt_field *hostname_field =
      bt_field_structure_borrow_member_field_by_index_const(common_context_field, 0);
  hostname = std::string{bt_field_string_get_value(hostname_field)};

  const bt_field *process_id_field =
      bt_field_structure_borrow_member_field_by_index_const(common_context_field, 1);
  process_id = bt_field_integer_signed_get_value(process_id_field);

  const bt_field *thread_id_field =
      bt_field_structure_borrow_member_field_by_index_const(common_context_field, 2);
  ...
  for (uint64_t i = 0; i < message_count; i++) {
    ...
    if (bt_message_get_type(message) == BT_MESSAGE_TYPE_EVENT) {
      ...
      if (strcmp(class_name, "lttng:host") == 0) {
        ...
        TallyCoreTime a{(uint64_t)dur, err};
        dispatch->device2[hpt_device_function_name_t(hostname, process_id, thread_id, did, sdid,
                                                     (thapi_function_name)name)] += a;
        ...
        dispatch->traffic2[hpt_function_name_t(hostname, process_id, thread_id, name)] += a;
...
tally_dispatch_initialize(bt_self_component_sink *self_component_sink,
  ...
  const std::string display_mode(val && bt_value_is_string(val) ? bt_value_string_get(val)
                                                                : "compact");
  val = bt_value_map_borrow_entry_value_const(params, "name");
  const std::string display_name(val && bt_value_is_string(val) ? bt_value_string_get(val)
                                                                : "mangle");
  val = bt_value_map_borrow_entry_value_const(params, "display_mode");
  const std::string display_human(val && bt_value_is_string(val) ? bt_value_string_get(val)
                                                                 : "human");
  ...
  const int display_name_max_size =
      (val && bt_value_is_signed_integer(val) ? bt_value_integer_signed_get(val) : -1);
  ...
  if (dispatch->display_human) {
    ...
    if (dispatch->display_compact) {
      ...
      print_compact("Device profiling", dispatch->device2,
                    std::make_tuple("Hostnames", "Processes", "Threads", "Device pointers",
                                    "Subdevice pointers"),
                    max_name_size);
      ...
      print_extended("API calls", dispatch->host2, std::make_tuple("Hostname", "Process", "Thread"),
                     max_name_size);

      print_extended(
          "Device profiling", dispatch->device2,
    ...
    if (dispatch->display_compact) {
      ...
      if (!dispatch->host2.empty())
        j["host"] =
            json_extented(dispatch->host2, std::make_tuple("Hostname", "Process", "Thread"));
      if (!dispatch->device2.empty())
      ...
      if (!dispatch->traffic2.empty())
        j["traffic"] =
            json_extented(dispatch->traffic2, std::make_tuple("Hostname", "Process", "Thread"));
    }
    std::cout << j << std::endl;
  }
}
